	...
	public Browsers(BrowsersType browserstype){
		switch(browserstype){
		    case firefox:
		    	...
		    	File firebug = new File(projectpath+"/tool/firebug-1.12.8.1-signed.xpi");
			    File firepath = new File(projectpath+"/tool/FireXPath-0.9.7.1.1-signed.1-signed.xpi");
				firefoxprofile =  new FirefoxProfile();
				try {
					firefoxprofile.addExtension(firebug);
					firefoxprofile.addExtension(firepath);
					firefoxprofile.setPreference("webdriver.accept.untrusted.certs", "true"); 
					firefoxprofile.setPreference("extensions.firebug.currentVersion", "1.12.8.1-signed");
				} catch (IOException e) {
